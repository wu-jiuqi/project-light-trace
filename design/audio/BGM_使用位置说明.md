...
### 4. `bgm_fragment_0001_loop.ogg` — 碎片 #0001「启程之镇」探索

| 属性 | 值 |
|------|-----|
| 源文件 | `assets/audio/0002.mp3` |
| 原始规格 | 146.4秒 / 44100Hz / 立体声 / 192kbps / 输入响度 -12.0 LUFS / +0.8 dBTP（削波） |
| 全曲版 | `bgm_fragment_0001.ogg`（2:26，全曲保留） |
| 循环版 | `bgm_fragment_0001_loop.ogg`（2:23，2s淡入+尾声3s淡出，去除极端入出尾） |
| 响度标准化 | -20 LUFS（Integrated），-1.0 dBTP |
| 播放场景 | `fragment_0001.tscn` |
| 触发时机 | 碎片场景加载完成、SceneFader 淡入后自动播放 |
| 停止时机 | ①返回星图（胜利画面按钮）②合规度归零强制返回 |
| 脚本位置 | `scripts/fragment/fragment_0001.gd` — `_start_bgm()` / `_stop_bgm()` |
| 音量 | -10 dB（比星图更低——碎片#0001有大量环境SFX：钟声、日晷摩擦、光墙电流、数据板蜂鸣等） |

**设计意图**：此 BGM 为极简环境纹理风格（ambient/drone），以极低响度和宽动态范围作为碎片探索的氛围层，不与交互 SFX 竞争听觉注意。

**与设计文档声音元素的共存**：

| 设计文档声音元素 | BGM 处理 |
|---|---|
| 远城市白噪音（环境 SFX） | BGM 不包含城市白噪——由游戏 SFX 系统独立播放 |
| 喷泉水声 8s 断点 | BGM 非常安静（-20 LUFS），不干扰喷泉循环的叙事意义 |
| 50Hz 光墙电流声 | BGM 不含 50Hz 元素——光墙 SFX 独占此频率 |
| 日晷石金属摩擦（交互 SFX） | BGM 不包含——交互时刻 SFX 清晰可辨 |
| 暗室地鸣（18Hz） | BGM 不含——暗室 SFX 独立触发 |
| 数据板蜂鸣（林指导） | BGM 不含——NPC 对话 SFX 独立 |
