...
func _ready() -> void:
	...
	if _frame_paths.is_empty():
		push_error("[Fragment0004Transition] No frames found in %s" % FRAME_DIR)
		_finish_transition()
		return
	_show_frame(0)
	AudioManager.play_voice("fragment_0004_transition", audio_player.stream, AudioManager.PRIORITY_HIGH, -8.0, 0.0)
	_fade_in()
