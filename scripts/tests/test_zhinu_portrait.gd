...
func _run() -> void:
	...
	for index in range(game_manager.awakened_colors.size()):
		game_manager.awakened_colors[index] = true
	...
	_check(game_manager.gray_cloth_uncovered, "uncover state keeps the legacy save key")
