...
func _update_detail_panel() -> void:
	...
	if not f.implemented:
		hint_label.text = "该碎片仍在构建中。\n当前 MVP 可进入碎片：#0762「颜色的葬礼」。"
		decrypt_btn.disabled = true
		enter_btn.disabled = true
		return
	...
	match f.decrypt_state:
		FragmentManager.DecryptState.LOCKED:
			hint_label.text = "[锁定] 碎片加密中 - 表面流动着乱码...\n点击「提交解密」请求天枢公司科研团队解密"
...
func _process(_delta: float) -> void:
	...
	for f in FragmentManager.fragments:
		if f.decrypt_state in [FragmentManager.DecryptState.DECRYPTING, FragmentManager.DecryptState.PARTIAL]:
			var previous_state = f.decrypt_state
			FragmentManager.check_decrypt_progress(f)
			if f.decrypt_state != previous_state:
				update_fragment_list()
